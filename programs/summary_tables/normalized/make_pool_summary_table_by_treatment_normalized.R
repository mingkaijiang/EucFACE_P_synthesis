...
make_pool_summary_table_by_treatment_normalized <- function() {
    ...
    ### canopy litter P
    out <- summaryBy(predicted~Ring,data=canopy_litter_p_pool_pred,FUN=mean,keep.names=T,na.rm=T)
    treatDF[treatDF$terms == "Canopy Litter P Pool", 2:7] <- out$predicted
    ...
    ### Understorey Litter P pool
    out <- summaryBy(predicted~Ring,data=understorey_litter_p_pool_pred,FUN=mean,keep.names=T,na.rm=T)
    treatDF[treatDF$terms == "Understorey Litter P Pool", 2:7] <- out$predicted
    ...
    #out <- summaryBy(predicted~Ring,data=soil_exchangeable_pi_pool_pred,FUN=mean,keep.names=T,na.rm=T)
    #treatDF[treatDF$terms == "Exchangeable Pi Pool", 2:7] <- out$predicted
    #
    #### Exchangeable Po Pool
    #out <- summaryBy(predicted~Ring,data=soil_exhangeable_po_pool_pred,FUN=mean,keep.names=T,na.rm=T)
    #treatDF[treatDF$terms == "Exchangeable Po Pool", 2:7] <- out$predicted
    #
    #
