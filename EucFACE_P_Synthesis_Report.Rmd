...
### 5.2 Soil P mineralization rate
``` {r, echo=FALSE, warning=FALSE}
    ...
    p <- ggplot(soil_p_mineralization, aes(date, p_mineralization_g_m2_d, color=factor(CO2))) +   
        geom_point(size = 5) +
...
### 5.3 Leaf P retranslocation coefficient
We need to think about data collected from the same time point. 


### 5.4 C fraction for frass production
``` {r, echo=FALSE, warning=FALSE}
...
Note, at this stage in time, not sure whether this quarterly measured value should has the unit of /d, or /quarter. 
I think it should not be a quarterly cumulative value, but neither should it be a daily value. 
``` {r, echo=FALSE, warning=FALSE}
    ...
    ### make the plot
    p <- ggplot(soil_phosphate_production,
