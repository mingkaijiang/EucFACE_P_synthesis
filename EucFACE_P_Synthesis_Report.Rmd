...
Now, I calculate total standing P stocks
...
``` {r, echo = FALSE} 
    ...
    ### make the plot
    p <- ggplot(standing_p_stock[,c("Ring", "Year", "total")],
                aes(Ring, total)) +   
            geom_bar(stat = "identity", aes(fill = as.factor(Year)), position = "dodge") + 
            xlab("Ring") + ylab("Standing P stock (g P m-2)")
    plot(p)
