...
``` {r, echo = FALSE} 
    ...
    # make the table looks nicer
    colnames(summary_table_flux_by_treatment)[1] <- "term"
...
Now, I calculate overstorey standing P stocks

``` {r, echo = FALSE} 
    source("programs/make_overstorey_standing_p_stock.R")
    standing_p_stock <- make_overstorey_standing_p_stock(leaf=canopy_p_pool, 
                                              wood=wood_p_pool, 
...
``` {r, echo = FALSE} 
    ...
    # make the table looks nicer
    colnames(summary_table_p_budgets)[1] <- "Variable"

    kable(summary_table_p_budgets)

```

Note: total P retranslocation includes leaf and fineroot retranslocation, plus wood P increment. It currently does not include any components of the understorey parts. So, the uptake/requirement ratio is high because we ignores understorey retranslocation. Or, we should only be calculating the overstorey P uptake and requirement. 

There is no literature value on Microlaena P retranslocation. 
...
# 8. To-do list
    ...
    - Check microbial pool biomass data with Cat
    - Wood C production and pool over time, why negative
    - Create mortality pool, or standing dead pool
    - summary diagrams for aCO2 and eCO2
    - fraction of sapwood to extrapolate wood retranslocation coefficients
    - fine root and coarse root P retranslocation coefficients
    - understorey respiration from literature
    - CH4 data
    - litter pool
