make_understorey_aboveground_c_pool_camera <- function(c_frac, plot.option) {
    ...
    if (plot.option == T) {
        ...
        p1 <- ggplot() +
            geom_line(tmpDF, mapping=aes(x=Date, y=percent_live, col=Ring))+
            geom_point(pctDF, mapping=aes(x=Date, y=percent_live,
                                          fill=Ring), 
                       size=2, pch=21)
        
        pdf("plots_tables/checks/interpolated_understorey_pct_live.pdf")
        plot(p1)
        dev.off()
