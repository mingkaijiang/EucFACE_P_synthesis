...
make_soil_n_mineralization_flux <- function(bk_density) {
    ...
    myDF1.m <- summaryBy(N_mineralisation~date+ring,data=myDF1,FUN=mean,keep.names=T,na.rm=T)
    ...
    myDF1.m$soil_n_mineralization <- myDF1.m$N_mineralisation * myDF1.m$bk_density * 0.1 

    # output table
    myDF1.out <- myDF1.m[,c("date", "ring", "soil_n_mineralization")]
    colnames(myDF1.out) <- c("Date", "Ring", "soil_n_mineralization_flux")
